...
function getImage(rating){
    if (!rating||rating >= 2800||getConfig("no_image")) return "";
    let ans = `<img src="`;
    ans += `//img.atcoder.jp/assets/user/user-`;
    //user-orange-3.png

    ans += getColor(rating);
    ans += `-`;
    ans += Math.floor(rating % 400 / 100) + 1;
...
function getDefferedRatingElem(result) {
    ...
    async function listener() {
        ...
        try {
            ...
            elem.append(getSpan(["加载失败"], ["red"]), document.createElement("br"), getSpan(["(悬停以查看详细信息)"], ["grey", "small"]), getSpan([e.toString()], ["my-tooltiptext"]));
    ...
    async initialize() {
        ...
        this.standingsTableView = StandingsTableView.Get(async (userScreenName) => {
            if (!this.ratingProvider)
                return { "type": "error", "message": "无法计算rating" };
            if (!this.performanceProvider)
            ...
            if (this.isRatedMaps.get(userScreenName)) {
                if (!this.ratingProvider.provider.availableFor(userScreenName))
                    return { "type": "error", "message": `无法获取${userScreenName}的rating数据` };
