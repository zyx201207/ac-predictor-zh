...
function getDifferenceString(newr,oldr){
    if (newr == oldr) return "±0";
    return toSignedString(newr - oldr);
}
function getRatedRatingElem(result) {
    const elem = document.createElement("div");
    elem.append(getRatingSpan(result.oldRating), getSpan(result.oldRating == result.newRating?" → ":(result.oldRating < result.newRating?" ↗ ":" ↘ "),[(result.oldRating == result.newRating?"grey":(result.oldRating < result.newRating?"green":"red"))]), getRatingSpan(result.newRating), " ", getSpan([`(${getDifferenceString(result.newRating,result.oldRating)})`],[(result.oldRating == result.newRating?"grey":(result.oldRating < result.newRating?"green":"red"))]));
    return elem;
}
...
class CalcRatingModel extends EstimatorModel {
    constructor(inputValue, perfHistory) {
        ...
        this.inputDesc = "预期Performance";
        this.resultDesc = "Rating";
    }
    // @ts-ignore
    toggle() {
...
class CalcPerfModel extends EstimatorModel {
    constructor(inputValue, perfHistory) {
        super(inputValue, perfHistory);
        this.inputDesc = "目标Rating";
        this.resultDesc = "所需Perf";
    }
    // @ts-ignore
    toggle() {
...
class EstimatorElement {
    ...
    async afterOpen() {
        const colorNames = ["unrated", "gray", "brown", "green", "#00C0C0", "blue", "#C0C000", "orange", "red"];
        function getColor(rating) {
            const colorIndex = rating > 0 ? Math.min(Math.floor(rating / 400) + 1, 8) : 0;
            return colorNames[colorIndex];
        }
        ...
        function updateView() {
            ...
            document.getElementById("estimator-input").style.color = getColor(model.inputValue);
            document.getElementById("estimator-res").style.color = getColor(model.resultValue);
